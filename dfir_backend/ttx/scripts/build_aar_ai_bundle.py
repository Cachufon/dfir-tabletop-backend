#!/usr/bin/env python3
"""
Build a single AI input bundle for drafting a TTX AAR.

This does NOT call any AI. It creates a text file you can paste into your approved LLM workflow.

Bundle includes:
- Prompt template: dfir_backend/ai_assist/prompts/ttx_aar_from_scribe_notes.md
- Scenario YAML (full text)
- scribe_runtime.json (full text)
- Optional: IR plan text file (client-provided)
...
Example:
  python3 dfir_backend/ttx/scripts/build_aar_ai_bundle.py \
    ...
    --ir-plan /secure_storage/ttx/<case_id>/10_inputs/ir_plan.txt \
    --intake-notes /secure_storage/ttx/<case_id>/10_inputs/intake_notes.txt \
...
def repo_root_from_here() -> Path:
    # .../dfir_backend/ttx/scripts/build_aar_ai_bundle.py -> repo root is parents[3]
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Build a paste-ready AI bundle for TTX AAR drafting.")
    ...
    prompt_path = repo_root / "dfir_backend" / "ai_assist" / "prompts" / "ttx_aar_from_scribe_notes.md"
    ...
    out_path = out_dir / "aar_ai_bundle.txt"
